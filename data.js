...
## Классические категории Top 10:2021
- **Broken Access Control** - ошибки в правах доступа: пользователь получает чужие данные или вызывает операции, которые ему не положены.
- **Cryptographic Failures** - проблемы с шифрованием: чувствительные данные хранятся или передаются без достаточной защиты.
- **Injection** - внедрение вредоносных команд: приложение опасно смешивает пользовательский ввод с SQL, shell, LDAP, шаблонами или другими интерпретаторами.
- **Insecure Design** - небезопасный дизайн: риск заложен в самой модели системы, бизнес-потоке или архитектурном решении ещё до конкретной реализации.
- **Security Misconfiguration** - неправильные настройки безопасности: дефолтные пароли, лишние сервисы, подробные ошибки, слабые заголовки и небезопасные cloud-настройки.
- **Vulnerable and Outdated Components** - уязвимые или устаревшие компоненты: проект опирается на библиотеки, контейнеры или фреймворки с известными проблемами безопасности.
- **Identification and Authentication Failures** - ошибки аутентификации: слабые пароли, плохая защита сессий, обход логина, проблемы с MFA и lifecycle учётных данных.
- **Software and Data Integrity Failures** - нарушения целостности ПО и данных: обновления, зависимости, CI/CD или внешние данные принимаются без достаточной проверки на подмену.
- **Security Logging and Monitoring Failures** - проблемы с логированием и мониторингом: атака происходит, но команда не видит сигналов, не поднимает alert и не может быстро расследовать инцидент.
- **Server-Side Request Forgery (SSRF)** - подделка серверных запросов: злоумышленник заставляет сервер ходить во внутренние сети, metadata endpoints или доверенные внешние системы.

## Что изменилось в Top 10:2025
- **SSRF** больше не отдельный пункт и рассматривается внутри **Broken Access Control**.
- **Vulnerable and Outdated Components** расширили до **Software Supply Chain Failures**: фокус теперь не только на версиях библиотек, но и на build pipeline, registry, пакетах и каналах поставки.
- **Identification and Authentication Failures** упростили по названию до **Authentication Failures**.
- **Security Logging and Monitoring Failures** переименовали в **Security Logging and Alerting Failures**, чтобы подчеркнуть важность не только логов, но и реакции на них.
- В список вошла новая категория **Mishandling of Exceptional Conditions**: ошибки обработки исключений, fail-open сценарии и некорректное поведение в нештатных условиях.
