    ...
    rows: [
          ...
          link("Efficient querying", "https://learn.microsoft.com/en-us/ef/core/performance/efficient-querying")
            ...
            topic("HiLo генерация ключей", md(
              '## Обзор',
              'HiLo - это стратегия генерации числовых идентификаторов блоками. Приложение получает у базы не одно следующее значение, а диапазон значений и затем использует его локально без round-trip на каждый `INSERT`.',
              '',
              'В EF Core это обычно реализуется через sequence-backed value generation и провайдерные настройки вроде `UseHiLo()`.',
              '',
              '## Пример для SQL Server',
              '```csharp',
              'protected override void OnModelCreating(ModelBuilder modelBuilder)',
              '{',
              '    modelBuilder.UseHiLo("EntityHiLoSequence");',
              '}',
              '```',
              '',
              '## Когда это полезно',
              '- много вставок и не хочется зависеть от `IDENTITY` на каждый insert',
              '- идентификатор нужен объекту до `SaveChanges()`',
              '- приложение создает целые графы объектов и хочет раздать ключи заранее',
              '',
              '## Trade-offs',
              '- стратегия обычно провайдер-специфична',
              '- в последовательности будут пропуски, и это нормально',
              '- для внешнего мира такой идентификатор все равно не стоит трактовать как непрерывный счетчик без дыр',
              '',
              '## С чем сравнивать',
              '| Подход | Плюс | Минус |',
              '| --- | --- | --- |',
              '| `IDENTITY` | просто и привычно | база участвует в генерации на каждом insert |',
              '| HiLo | меньше round-trips, ключ известен заранее | сложнее настройка, обычно только для sequence-friendly провайдеров |',
              '| `Guid` | удобно для распределенных систем | хуже индексная локальность, если GUID случайный |',
              '',
              '## Практика',
              'HiLo - это не обязательная оптимизация для каждой системы. Обычно к нему приходят там, где уже видно реальную цену генерации ключей через базу или нужно строить большие графы сущностей до сохранения.'
            ), [
          ...
          link("SQL Server value generation", "https://learn.microsoft.com/en-us/ef/core/providers/sql-server/value-generation")
            ...
            topic("Concurrency Tokens", md(
              ...
              '',
              '## Обзор',
              'Concurrency token нужен для optimistic concurrency. EF Core запоминает значение токена при чтении строки, а при `UPDATE` или `DELETE` добавляет его в `WHERE`. Если строка уже изменилась кем-то еще, affected rows окажется равным нулю, и вы получите `DbUpdateConcurrencyException`.',
              '',
              '## Пример',
              '```csharp',
              ...
              '',
              '## Что это решает',
              '- два пользователя редактируют одну запись одновременно',
              '- фоновый процесс и API обновляют одну и ту же строку',
              '- важна защита от тихого перетирания чужих изменений последней записью',
              '',
              '## Что делать при конфликте',
              '| Стратегия | Когда подходит |',
              '| --- | --- |',
              '| reload + retry | конфликт технический и его можно автоматически повторить |',
              '| показать конфликт пользователю | редактирование человеко-ориентированное и решение должен принять пользователь |',
              '| merge изменений | нужно аккуратно объединить независимые поля |',
              '',
              '## Ограничения',
              '- optimistic concurrency не блокирует строку заранее',
              '- токен защищает только те операции, где вы действительно сохраняете сущность через EF и проверяете исключение',
              '- нужен понятный сценарий обработки конфликта, иначе токен лишь переносит проблему из базы в код приложения',
              '',
              '## Практика',
              'Если запись может редактироваться конкурентно, concurrency token обычно дешевле и чище, чем тяжелые блокировки на чтении. Но это работает только тогда, когда команда понимает, что делать после конфликта.'
            ), [
          ...
          link("Lazy loading of related data", "https://learn.microsoft.com/en-us/ef/core/querying/related-data/lazy")
            ...
            topic("Split Queries", md(
              ...
              '',
              '## Обзор',
              'По умолчанию `Include` часто приводит к одному большому SQL-запросу с JOIN. Для простых графов это нормально, но при нескольких коллекциях может появиться cartesian explosion: количество строк резко растет, а данные родительской сущности дублируются снова и снова.',
              '',
              '`AsSplitQuery()` разрешает EF Core разбить такой запрос на несколько SQL-команд и потом собрать граф в памяти.',
              '',
              '## Когда это помогает',
              '- в графе несколько коллекций',
              '- один большой JOIN дублирует широкие колонки сотни или тысячи раз',
              '- single query уже видимо распух по количеству строк и памяти',
              '',
              '## Цена решения',
              '- больше одного round-trip к базе',
              '- возможны нюансы консистентности между запросами, если данные параллельно меняются',
              '- это не бесплатное ускорение, а обмен формы одного запроса на несколько более узких',
              '',
              '## Практика',
              'Если проблема именно в раздутом JOIN и дублировании данных, split query часто помогает. Если же проблема в количестве запросов как таковом, он может сделать только хуже. Сначала смотрите на фактический SQL и объем данных, а не на красивость API-вызова.'
            ), [
          ...
          link("What's New in EF Core 5.0", "https://learn.microsoft.com/en-us/ef/core/what-is-new/ef-core-5.0/whatsnew")
            ...
            topic("Функции базы данных", md(
              '## Обзор',
              'Если часть вычислений уже живет в SQL-функции и ее нужно вызывать из LINQ, EF Core позволяет сопоставить .NET-метод с database function через `HasDbFunction`. Тогда функция начинает участвовать в SQL-переводе запроса.',
              '',
              '## Когда это оправдано',
              '- логика уже существует в БД и ей доверяют',
              '- выражение трудно или невозможно перевести стандартным LINQ',
              '- важно выполнять вычисление на стороне сервера, а не после materialization',
              '',
              '## Пример идеи',
              '```csharp',
              'public static int ActiveDays(DateTime from, DateTime to)',
              '    => throw new NotSupportedException();',
              '',
              'modelBuilder.HasDbFunction(typeof(AppDbContext)',
              '    .GetMethod(nameof(ActiveDays))!);',
              '```',
              '',
              'После этого метод можно использовать в LINQ как часть выражения, которое уйдет в SQL.',
              '',
              '## Где граница',
              '- если выражение нормально пишется в LINQ, чаще лучше оставить его в LINQ',
              '- database function - это мост к серверной логике, а не способ унести весь query layer в SQL',
              '- если нужен произвольный SQL-результат, смотрят уже на `FromSql`, `SqlQuery` или view',
              '',
              '## Практика',
              'Хороший критерий такой: функция должна быть маленькой, стабильной и реально ценной на стороне базы. Иначе код быстро становится зависимым не от модели, а от скрытой логики внутри SQL-сервера.'
            ), [
